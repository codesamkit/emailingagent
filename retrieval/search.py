...
def _embed_query(query: str) -> Optional[bytes]:
    ...
    reaching into llm.embeddings, which doesn't exist in this repo yet
    (Person A's A2 — see interfaces/README.md). Returns None when that
    module isn't available; the vector channel then contributes nothing,
    same graceful-degradation shape as every other guarded cross-track
    import in this build (see pipeline/orchestrate.py's with_defaults)."""
    try:
